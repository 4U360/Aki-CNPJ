import pathlib
import unittest
from akicnpj.downloader import AkiSituacaoCadastralDownloader
from akicnpj.extractor import AkiExtractor
from akicnpj.object.table.situacao_cadastral import AkiSituacaoCadastral
...
class SituacaoCadastralTestCase(unittest.TestCase):
    zip_files = []

    def test_download(self):
        files = list(AkiSituacaoCadastralDownloader().download())
    ...
    def test_find_and_read(self):
        for file in AkiFinder(path=EXTRACT_PATH).search():
            if file.category == file.category.CADASTRAL_SITUATION:
                with AkiSituacaoCadastralReader(file.path, encoding="iso-8859-1") as reader:
                    for row in reader.rows:
                        self.assertIsInstance(row, AkiSituacaoCadastral)
