import pathlib
import unittest
from akicnpj.downloader import AkiQSADownloader
from akicnpj.extractor import AkiExtractor
from akicnpj.object.table.qsa import AkiQSA
from akicnpj.reader import AkiQSAReader
...
class QSATestCase(unittest.TestCase):
    zip_files = []

    def test_download(self):
        files = list(AkiQSADownloader().download())
    ...
    def test_find_and_read(self):
        for file in AkiFinder(path=EXTRACT_PATH).search():
            if file.category == file.category.CORPORATE_STRUCTURE:
                with AkiQSAReader(file.path, encoding="iso-8859-1") as reader:
                    for row in reader.rows:
                        self.assertIsInstance(row, AkiQSA)
