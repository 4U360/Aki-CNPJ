import pathlib
import unittest
from akicnpj.downloader import AkiSocioDownloader
from akicnpj.extractor import AkiExtractor
from akicnpj.object.socios import AkiSocio
...
class SociosTestCase(unittest.TestCase):
    ...
    def test_download(self):
        files = list(AkiSocioDownloader().download())
    ...
    def test_find_and_read(self):
        for file in AkiFinder(path=EXTRACT_PATH).search():
            if file.category == file.category.PARTNER:
                with AkiSociosReader(file.path, encoding="iso-8859-1") as reader:
                    for row in reader.rows:
                        self.assertIsInstance(row, AkiSocio)
