...
from ..settings import ROOT, Path,AKI_FILE_PATTERN_MUNICIPIOS


class AkiMunicipioDownloader(AkiDownloader):

    def download(self, path: Path = ROOT.parent.joinpath("data"), ignore_exists: bool = False):
        makedirs(str(path), exist_ok=True)

        for file in self.files:
            if bool(AKI_FILE_PATTERN_MUNICIPIOS.match(file.name)):
